...
from sklearn import preprocessing, model_selection, feature_extraction, feature_selection, metrics, manifold, \
...
def plot_distributions(dtf, x, max_cat=20, top=None, y=None, bins=None, figsize=(10, 5), outputFile=None):
    ## univariate
    if y is None:
        ...
        if dtf[x].nunique() <= max_cat:
            if top is None:
                dtf[x].reset_index().groupby(x).count().sort_values(by="index").plot(kind="barh", legend=False,
                                                                                     ax=ax).grid(axis='x')
            else:
                dtf[x].reset_index().groupby(x).count().sort_values(by="index").tail(top).plot(kind="barh",
                                                                                               legend=False,
                                                                                               ax=ax).grid(axis='x')
            ax.set(ylabel=None)
        ### numerical
    ...
    ## bivariate
        ...
        for i in dtf[y].unique():
            sns.distplot(dtf[dtf[y] == i][x], hist=True, kde=False, bins=bins, hist_kws={"alpha": 0.8}, axlabel="",
                         ax=ax[0])
            # https://stackoverflow.com/questions/63513328/how-to-resolve-sns-pairplot-error-selected-kde-bandwidth-is-0-cannot-estimate
            # -d#:~:text=Following%20Error%20with%20the%20OUTPUT%3A%20RuntimeError%3A%20Selected%20KDE,bw%20is%20the%20bandwidth.%20You%20should%20adjust%20bw.
            # JHP: Added the bw=0.3 for kde_kws.  Got exception otherwise though the "auto" or default seemed better.
            sns.distplot(dtf[dtf[y] == i][x], hist=False, kde=True, kde_kws={"shade": True, 'bw': 0.3}, axlabel="",
                         ax=ax[1])
...
'''
Removes nouns (NNP and NNPS) from the str.  Returns str without proper nouns. 
'''
...
def add_text_length(data, column):
    ...
    print(dtf[['char_count', 'word_count', 'sentence_count', 'avg_word_length', 'avg_sentence_length']].describe().T[
              ["min", "mean", "max"]])
    return dtf
...
def add_preprocessed_text(data, column, lst_regex=None, punkt=False, lower=False, lst_stopwords=None, stemm=False,
                          lemm=False, remove_na=True):
    dtf = data.copy()

    ## apply preprocess
    dtf = dtf[pd.notnull(dtf[column])]
    dtf[column + "_clean"] = dtf[column].apply(
        lambda x: utils_preprocess_text(x, lst_regex, punkt, lower, lst_stopwords, stemm, lemm))
...
def utils_ner_text(txt, ner=None, lst_tag_filter=None, grams_join="_"):
    ...
    for tag in entities:
        if (lst_tag_filter is None) or (tag.label_ in lst_tag_filter):
            try:
                tagged_txt = re.sub(tag.text, grams_join.join(tag.text.split()),
                                    tagged_txt)  # it breaks with wild characters like *+
            except Exception as e:
                continue

    ## extract tags list
    if lst_tag_filter is None:
        lst_tags = [(tag.text, tag.label_) for tag in
                    entities]  # list(set([(word.text, word.label_) for word in ner(x).ents]))
    else:
...
def add_ner_spacy(data, column, ner=None, lst_tag_filter=None, grams_join="_", create_features=True):
    ...
    dtf[[column + "_tagged", "tags"]] = dtf[[column]].apply(
        lambda x: utils_ner_text(x[0], ner, lst_tag_filter, grams_join),
    ...
    try:
        ...
        with ner_model.disable_pipes(*other_pipes):
            ...
            for n in range(n_iter):
                ...
                losses = {}
                batches = spacy.util.minibatch(train_data, size=spacy.util.compounding(4., 32.,
                                                                                       1.001))  ###batch up data using spaCy's minibatch
...
:parameter
    ...
    :param resultFile: InStream to write results to
'''


def evaluate_multi_classif(y_test, predicted, predicted_prob, outputDir, resultFile):
...
def fit_bow(corpus, vectorizer=None, vocabulary=None):
    ...
    vectorizer = feature_extraction.text.TfidfVectorizer(max_features=None, ngram_range=(1, 1),
                                                         vocabulary=vocabulary) if vectorizer is None else vectorizer
    ...
    sns.heatmap(X.todense()[:, np.random.randint(0, X.shape[1], 100)] == 0, vmin=0, vmax=1, cbar=False,
                ax=ax).set_title('Sparse Matrix Sample')
    plt.show()
...
:parameter
    ...
    :param htmlfilename: string name of html file to save
:return
    dtf with explanations
'''


def explainer_lime(model, y_train, txt_instance, top=10, htmlfilename='nlp_utils.html'):
    ...
    explained.save_to_file(htmlfilename, text=txt_instance, predict_proba=False)
...
def create_ngrams_detectors(corpus, outputDir, grams_join=" ", lst_common_terms=[], min_count=5, top=10,
                            figsize=(10, 7)):
    ## fit models
    ## JHP I think common_terms was renamed to connector_words
    ##     Also expect str not bytes object for grams_join, so do not use grams_join.encode()
    lst_corpus = utils_preprocess_ngrams(corpus, ngrams=1, grams_join=grams_join)
    bigrams_detector = gensim.models.phrases.Phrases(lst_corpus, delimiter=grams_join, connector_words=lst_common_terms,
    ...
    trigrams_detector = gensim.models.phrases.Phrases(bigrams_detector[lst_corpus], delimiter=grams_join,
                                                      connector_words=lst_common_terms,
                                                      min_count=min_count, threshold=min_count * 2)
    trigrams_detector = gensim.models.phrases.Phraser(trigrams_detector)

    ## plot
    dtf_ngrams = pd.DataFrame(
        [{"word": grams_join.join([gram for gram in k]), "freq": v} for k, v in trigrams_detector.phrasegrams.items()])
    ...
    ax.set(xlabel=None, ylabel=None, title="Most frequent words")
    ax.grid(axis="x")
    plt.savefig(os.path.join(outputDir, "n_gram_detector.png"))
    # plt.close(fig) # disables displaying
...
def fit_w2v(corpus, ngrams=1, grams_join=" ", lst_ngrams_detectors=[], min_count=1, size=300, window=20, sg=1,
            epochs=30):
    ...
    nlp = gensim.models.word2vec.Word2Vec(lst_corpus, vector_size=size, window=window, min_count=min_count, sg=sg,
                                          epochs=epochs)
    # nlp = gensim.models.word2vec.Word2Vec(lst_corpus,  window=window, min_count=min_count, sg=sg)
...
def fit_dl_classif(X_train, y_train, X_test, encode_y=False, dic_y_mapping=None, model=None, weights=None, epochs=10, batch_size=64):
    ## encode y
    if encode_y is True:
...
def explainer_attention(model, tokenizer, txt_instance, lst_ngrams_detectors=[], top=5, figsize=(5, 3)):
    ...
    lst_corpus = utils_preprocess_ngrams([re.sub(r'[^\w\s]', '', txt_instance.lower().strip())],
                                         lst_ngrams_detectors=lst_ngrams_detectors)
    ...
    weights = preprocessing.MinMaxScaler(feature_range=(0, 1)).fit_transform(np.array(weights).reshape(-1, 1)).reshape(
        -1)
...
def word_clustering(corpus, nlp=None, ngrams=1, grams_join=" ", lst_ngrams_detectors=[], n_clusters=3):
    ...
    if nlp is None:
        print("--- training W2V---")
        lst_corpus, nlp = fit_w2v(corpus, ngrams=ngrams, grams_join=grams_join,
                                  lst_ngrams_detectors=lst_ngrams_detectors,
    ...
    kmeans_model = nltk.cluster.KMeansClusterer(n_clusters, distance=nltk.cluster.util.cosine_distance, repeats=50,
    ...
    dtf_clusters = pd.DataFrame(
        {"word": word, "cluster": str(clusters[i])} for i, word in enumerate(list(nlp.vocab.keys())))
    ...
    try:
        ...
        ## plot 3d
            ...
            for n, k in enumerate(dic_words.keys()):
                ax.scatter(dtf[dtf["cluster"] == k]['x'], dtf[dtf["cluster"] == k]['y'], dtf[dtf["cluster"] == k]['z'],
                           c=colors[n], label=k)
            ax.set(xlabel=None, ylabel=None, zlabel=None, xticklabels=[], yticklabels=[], zticklabels=[])
            ax.legend()
            if annotate is True:
...
def tokenize_bert(corpus, tokenizer=None, maxlen=None):
    tokenizer = transformers.BertTokenizer.from_pretrained('bert-base-uncased',
                                                           do_lower_case=True) if tokenizer is None else tokenizer
    input_dict = tokenizer(list(corpus), truncation=True,
                           padding='max_length',
                           max_length=maxlen)
    ...
    for seq in txt2seq:
        tokens = seq.split(" ")
        # ISSUE: See https://github.com/huggingface/transformers/issues/10297
        # print('DEBUG: ' + str( tokens ) )
        # encodedSeq = tokenizer.encode(tokens)
        encodedSeq = tokenizer.__call__(tokens)
        # print('DEBUG: ' + str( encodedSeq ) )
        for index in range(len(encodedSeq['input_ids'])):
            encod_list = encodedSeq['input_ids'][index]
            if len(encod_list) > 3:
                encodedSeq['input_ids'][index] = encodedSeq['input_ids'][index][-3:]
            if len(encod_list) < 3:
                encodedSeq['input_ids'][index] = [0, 0, 0]
        idx.append(encodedSeq['input_ids'])
    ...
    # The idx is a dict {'input_ids': [[101, 101, 102], [101, 1996, 102], ...], 'attention_mask': [[1, 1, 1], [1, 1, 1], ...] }
    # But the code as written expects a list of list of ints (i.e. looks like the 'input_ids' value
    # 'attention_mask'
    # input_ids      = idx['input_ids']
    # attention_mask = idx['attention_mask']
    for v_list in idx:
        for v in v_list:
            if len(v) == 3:
                continue
...
def fit_bert_classif(X_train, y_train, X_test, y_test, encode_y=False, dic_y_mapping=None, model=None, epochs=1,
                     batch_size=64):
    ## encode y
    if encode_y is True:
        dic_y_mapping = {n: label for n, label in enumerate(np.unique(y_train))}
    ...
    # training = model.fit(x=X_train, y=y_train, batch_size=batch_size, epochs=epochs,
    #                      shuffle=True, verbose=verbose, validation_split=0.3)
...
def utils_cosine_sim(a, b, nlp=None):
    ...
    if (type(a) is str) or (type(b) is str):
        ...
        if (len(a.shape) == 1) and (len(a.shape) == 1):
            a = a.reshape(1, -1)
            b = b.reshape(1, -1)
            cosine_sim = metrics.pairwise.cosine_similarity(a, b)[0][
                0]  # np.dot(a,b)/(np.linalg.norm(a)*np.linalg.norm(b))
...
def explainer_similarity_classif(tokenizer, nlp, dic_clusters, txt_instance, token_level=False, top=5,
                                 figsize=(20, 10)):
    ## create embedding Matrix
    y = np.concatenate([embedding_bert(v, tokenizer, nlp) for v in dic_clusters.values()])
    X = embedding_bert(txt_instance, tokenizer, nlp) if token_level is True else embedding_bert(txt_instance, tokenizer,
                                                                                                nlp).mean(0).reshape(1,
                                                                                                                     -1)
    ...
    if token_level is True:
        ...
        for i in range(len(dtf)):
            ax.annotate(dtf.index[i], xy=(dtf["x"].iloc[i], dtf["y"].iloc[i]), xytext=(5, 2),
